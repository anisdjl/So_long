...
/*   Updated: 2026/02/04 17:12:25 by adjelili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_destroy_image(master->mlx, master->sprites->dos.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->face.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->droite.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->e_f.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->e_o.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->ennemi.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->gauche.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->gemme.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->sols.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->murs.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->marche_d.ptr_img);
	mlx_destroy_image(master->mlx, master->sprites->marche_d.ptr_img);
}
...
t_img	get_addr_img(t_master *master, t_map *map, char *filename)
{
	...
	img.ptr_img = mlx_xpm_file_to_image(master->mlx, filename, &img.longeur, &img.largeur); // cette fonction renvoie l'addrese de l'image au format xpm qu'on lui a donne tout en modifiant sa hauteur et sa largeur par reference 
	if (!img.ptr_img)
	{
		free(master);
		ft_free_map(map);
...
void	draw_map(t_master *master, t_map *map)
{
	...
	while (x < map->x)
	{
		...
		while (map->map[x][y])
		{
			...
			mlx_put_image_to_window(master->mlx, master->window, master->sprites->sols.ptr_img, y * 64, x * 64); // cette fonction met l'image dans la fenetre en commancant par le coin haut gauche 
			if (map->map[x][y] == '1')
				...
				mlx_put_image_to_window(master->mlx, master->window, master->sprites->e_o.ptr_img, y * 64, x * 64);
				...
				mlx_put_image_to_window(master->mlx, master->window, master->sprites->dos.ptr_img, y * 64, x * 64);
			else if (map->map[x][y] == 'P' && master->frame == 3 && master->walk == 0)
				mlx_put_image_to_window(master->mlx, master->window, master->sprites->droite.ptr_img, y * 64, x * 64);
			else if (map->map[x][y] == 'P' && master->frame == 3 && master->walk == 1)
				mlx_put_image_to_window(master->mlx, master->window, master->sprites->marche_d.ptr_img, y * 64, x * 64);
			else if (map->map[x][y] == 'P' && master->frame == 4 && master->walk == 0)
				mlx_put_image_to_window(master->mlx, master->window, master->sprites->gauche.ptr_img, y * 64, x * 64);
			else if (map->map[x][y] == 'P' && master->frame == 4 && master->walk == 1)
				mlx_put_image_to_window(master->mlx, master->window, master->sprites->marche_g.ptr_img, y * 64, x * 64);
